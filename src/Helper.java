...
public class Helper {
	...
	public static String toHex(byte[] data, int inOffset, int length) {
		String out = new String();
		int loop;
		for (loop = 0; loop < length; loop++)
			out = out + String.format("%02X", data[loop + inOffset]);
		return out;
	}
	
	public static String toHexfoApplet(byte[] data, int length) {
		String out = new String();
		out = "(byte)0x";
		int loop;
		for (loop = 0; loop < length; loop++)
			out = out + String.format("%02X", data[loop]) + ", (byte)0x";
		return out;
	}
	...
	public static String toHexApplet(byte[] data) {
		return toHexfoApplet(data, data.length);
	}
